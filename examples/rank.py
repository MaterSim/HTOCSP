...
from pyxtal.util import ase2pymatgen, parse_cif
...
parser.add_option("-u", "--unit", dest="unit", action='store_false', default=True,
                  help="unit conversioon, optional")
...
if not options.unit:
    print("Converting the energy to eV/mole")
    cifs, spgs = parse_cif(options.cif, spg=True)
    for id in range(len(engs)):
        spg = spgs[id]
        engs[id] *= len(Group(spg)[0])
    ...
    for id, cif in enumerate(cifs):
        ...
        try:
            ...
            if new_struc(xtal, xtals, 100):
                xtal.energy = engs[id]*sum(xtal.numMols)
                xtals.append(xtal)
                strs = xtal.get_1D_representation().to_string(eng=engs[id])
                label = "{:d}-SG-{:d}".format(count, xtal.group.number)
                f.writelines(xtal.to_file(header=label))
                if options.rank != 'energy':
                    strs += '{:7.3f}'.format(sims[id])
                print("{:4d} {:4d}".format(id, count)+strs)
                count += 1
                if count == cut:
                    break
        except:
